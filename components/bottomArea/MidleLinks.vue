<script></script>

<template>
  <div class="my-5 flex">
    <div class="w-60 text-center flex flex-col items-center">
      <div
        class="flex items-center justify-center w-[140px] h-[140px] rounded-full shadow-xl overflow-hidden"
      >
        <img src="../../assets/capaciacoes.png" class="" alt="" />
      </div>
      <h3 class="text-lg font-bold">Capacitações</h3>
      <div class="flex flex-col items-center relative">
        <span class="w-14 bg-teal-700 h-[3px] absolute top-0 z-10"></span>
        <span class="w-40 bg-gray-400 h-[1px] absolute top-[0.5px]"></span>
      </div>
      <small class="mt-2 text-xs"
        >Esteja sempre em volução com as palestras e orientações dos nossos
        profissionais</small
      >
    </div>
    <div class="w-60 text-center flex flex-col items-center mx-14">
      <div class="w-[140px] h-[140px] rounded-full shadow-xl overflow-hidden">
        <img src="../../assets/escalas.png" class="mt-1" alt="" />
      </div>
      <h3 class="text-lg font-bold">Escalas</h3>
      <div class="flex flex-col items-center relative">
        <span class="w-14 bg-teal-700 h-[3px] absolute top-0 z-10"></span>
        <span class="w-40 bg-gray-400 h-[1px] absolute top-[0.5px]"></span>
      </div>
      <small class="mt-2 text-xs"
        >Conheça todos os nosso treinamentos disponíeis</small
      >
    </div>
    <div class="w-60 text-center flex flex-col items-center">
      <div
        class="flex items-center justify-center w-[140px] h-[140px] rounded-full shadow-xl overflow-hidden"
      >
        <img src="../../assets/treinamentos.png" class="w-[90%] mb-3" alt="" />
      </div>
      <h3 class="text-lg font-bold">Treinamentos</h3>
      <div class="flex flex-col items-center relative">
        <span class="w-14 bg-teal-700 h-[3px] absolute top-0 z-10"></span>
        <span class="w-40 bg-gray-400 h-[1px] absolute top-[0.5px]"></span>
      </div>
      <small class="mt-2 text-xs"
        >Conheça todos os nosso treinamentos disponíeis</small
      >
    </div>
  </div>
</template>
